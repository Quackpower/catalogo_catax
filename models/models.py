...
class catalogos_reporte(models.Model):
    _name = 'catalogos_reporte'
    ...
    areas_categoria = fields.One2many('subcategorias_reporte.relacion_categoria_areas', 'id_categoria')
    ...
    def create(self, vals):
        ...
        _categoria = self.env['catalogos_reporte'].sudo().search([('clave','=', clave)])

        if _categoria:
            raise exceptions.UserError('La clave ya existe.')
        
        return super(catalogos_reporte, self).create(vals)

    
    def write(self, vals):
        #if 'clave' in vals:
        #    raise exceptions.Warning("Imposible modificar la clave de la Categoría ya que contiene reportes relacionados.")
        record = super(catalogos_reporte, self).write(vals)       
        return record     

class subcategorias_reporte(models.Model):
    _name = 'subcategorias_reporte'
    clave = fields.Char(string="Clave", required=True)
    name = fields.Char(string="Nombre subcategoría", required=True)
    categoria = fields.Many2one('catalogos_reporte', string="Categoría perteneciente", ondelete="cascade")
    ...
    activo   = fields.Boolean(string="Activo",  default=True)
    etiquetas = fields.One2many('subcategorias_reporte.etiquetas', 'id_tag')
    icono = fields.Binary(string="Icono para la categoría, se mostrara al usuario final")
    ...
    def create(self, vals):
        name = vals.get('name')
        clave = vals.get('clave')
        _subcategoria = self.env['subcategorias_reporte'].sudo().search([('clave','=', clave)])

        if _subcategoria:
            raise exceptions.UserError('La clave ya existe.')
        
        return super(subcategorias_reporte, self).create(vals)
    ...
    def write(self, vals):
        ...
        record = super(subcategorias_reporte, self).write(vals)       
        return record     

class subcategorias_reporte_etiquetas(models.Model):
    _name = 'subcategorias_reporte.etiquetas'
    _rec_name = 'etiqueta'

    id_tag = fields.Many2one('subcategorias_reporte', ondelete="cascade")
    ...
    def create(self, vals):
        try:
            return super(subcategorias_reporte_etiquetas, self).create(vals)
    ...
    def write(self, vals):
        try:
            return super(subcategorias_reporte_etiquetas, self).write(vals)
        except Exception as er:
            raise exceptions.Warning(str(er))

    
    def read(self, fields=None, load='_classic_read'):
        return super(subcategorias_reporte_etiquetas, self).read(fields, load=load)
class ayto_folios(models.Model):
    ...
    """
    _name = 'subcategorias_reporte.folios'             
    ...
    def get_folio(self, model_name, increase=True):
        ...
        """
        try:
            foliador = self.env['subcategorias_reporte.folios']
...
class relacion_categoria_areas(models.Model):
    _name = 'subcategorias_reporte.relacion_categoria_areas'
    _rec_name = 'id_depto'

    id_categoria = fields.Many2one("catalogos_reporte", string="Categoria", ondelete="cascade")
